...
<nav class="navbar navbar-default">
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <a class="navbar-brand" href="#">管理员车辆管理</a>
        </div>
...
<script>
    ...
    /* $("#bicyclesavebtn").click(function () {
         ...
         $.ajax({
             url: "${APP_PATH}/bicycleCRUD",
            type: "POST",
            data:$("#bicycleAddModal form").serialize(),

            success: function (result) {
                if (result.code == 200) {
                    $("#bicycleAddModal").modal("hide");
                    to_page(totalRecord);
                } else {
                    if (undefined != result.extend.err.bianhao) {
                        $("#bianhao_add_input").parent().addClass("has-error");
                        $("#bianhao_add_input").next("span").text("自行车编号重复");
                    }
                }
            }
        })
    ...
    /*$("#bicycle_update_btn").click(function () {
        ...
        /!*$.ajax({
            ...
            type:"POST",
            data:$("#bicycleUpdateModal form").serialize()+"&_method=PUT",
            success:function (result) {
                alert(result)
            }
        })
        ...
        $.ajax({
            url: "${APP_PATH}/bicycleupdateCRUD/" + $(this).attr("edit-id"),
            type: "POST",
            data: $("#bicycleUpdateModal form").serialize(),
            success: function (result) {
                $("#bicycleUpdateModal").modal("hide");
                to_page(currentPage);
            }
        })
    })*/
