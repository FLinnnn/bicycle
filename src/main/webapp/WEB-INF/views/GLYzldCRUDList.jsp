...
<script>
    ...
    function build_zld_table(result) {
        $("#zldtables tbody").empty();
        var zld = result.extend.info;
        $.each(zld, function (index, item) {
            var zldbianhaotd = $("<td></td>").append(item.bianhao);
            var zldmingchengtd = $("<td></td>").append(item.mingcheng);
            var zlddidiantd = $("<td></td>").append(item.didian);
            var zldimagetd = $("<img></img>").prop("src","images/"+item.zldimage);
            $("<tr></tr>").append(zldbianhaotd).append(zldmingchengtd).append(zlddidiantd)
                .append(zldimagetd).appendTo("#zldtables tbody")
        })
    }
    function to_page() {
        $.ajax({
            url: "${APP_PATH}/zldxinxi",
            type: "GET",
            success: function (result) {
                build_zld_table(result);
            }
        })
    };
