    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
            <ul class="nav navbar-nav">
                ...
                <li class="active"><a href="#" id="GLYyhListbtn">用户管理</a></li>
...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-4 col-md-offset-10">
            <button class="btn btn-success" id="yh_add_modal_btn"><span class="glyphicon glyphicon-plus"
                                                                            aria-hidden="true"></span>新增
            </button>
            <button class="btn btn-danger" id="yh_delect_all_btn"><span class="glyphicon glyphicon-scissors" aria-hidden="true"></span>删除
            </button>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <table class="table table-hover" id="yhtables">
                <thead>
                <tr>
...
<!-- 用户添加的模态框 -->
<div class="modal fade" id="yhAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span>
                </button>
                <h4 class="modal-title" id="myModalLabel">用户添加表单</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal" id="savenewyh">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="uname" class="form-control" id="name_add_input"
                                   placeholder=">请输入昵称">
                            <span class="help-block"></span>
                        </div>
                    </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="yhsavebtn">保存</button>
            </div>
        </div>
    </div>
</div>
<!-- 用户修改的模态框 -->
<div class="modal fade" id="yhUpdateModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span>
                </button>
                <h4 class="modal-title">用户修改表单</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal" id="updateyh">
                    ...
                    <div class="form-group">
                        <label class="col-sm-2 control-label">昵称</label>
                        <div class="col-sm-10">
                            <input type="text" name="uname" class="form-control" id="name_update_input"
                                   placeholder=">请输入昵称">
                            <span class="help-block"></span>
                        </div>
                    </div>
...
<script>
    ...
    function build_yh_table(result) {
        $("#yhtables tbody").empty();
        var yhCRUD = result.extend.pageInfo.list;
        $.each(yhCRUD, function (index, item) {
            var checkboxtd = $("<td><input type='checkbox' class='check_item'></td>");
            var uidtd = $("<td></td>").append(item.uid);
            var usernametd = $("<td></td>").append(item.username);
            var passwordtd = $("<td></td>").append(item.password);
            var nametd = $("<td></td>").append(item.uname);
    ...
    function to_page(pn) {
        $.ajax({
            url: "${APP_PATH}/yhListCRUD",
            data: "pn=" + pn,
            type: "GET",
            success: function (result) {
                build_yh_table(result);
    ...
    $("#yh_add_modal_btn").click(function () {
        reset_from("#yhAddModal form");
        getzldlist("#yhAddModal select");
        $("#yhAddModal").modal({
            backdrop: "static"
        })
    });
    ...
    $("#username_add_input").change(function () {
        ...
        if (!flag) {
            ...
            $("#yhsavebtn").attr("ajax-username", "error");
        }
        $.ajax({
            ...
            success: function (result) {
                if (result.code == 200) {
                    ...
                    $("#yhsavebtn").attr("ajax-username", "success");
                } else {
                    $("#username_add_input").parent().removeClass("has-success has-error");
                    $("#username_add_input").next("span").text("");
                    $("#username_add_input").parent().addClass("has-error");
    ...
    $("#yhsavebtn").click(function () {
        ...
        $.ajax({
            url: "${APP_PATH}/yhCRUD",
            type: "POST",
            data: $("#savenewyh").serialize(),
            success: function (result) {
                if (result.code == 200) {
                    $("#yhAddModal").modal("hide");
                    to_page(totalRecord);
                } else {
    ...
    $("#yh_update_btn").click(function () {
        ...
        $.ajax({
            url: "${APP_PATH}/yhCRUD/" + $(this).attr("edit-id"),
            type: "PUT",
            data: $("#yhUpdateModal form").serialize(),
            success: function (result) {
                $("#yhUpdateModal").modal("hide");
                to_page(currentPage);
            }
        })
    })
    ...
    $("#yh_delect_all_btn").click(function(){
        var yhnames="";
        var strids="";
        $.each($(".check_item:checked"),function () {
            yhnames +=$(this).parents("tr").find("td:eq(2)").text()+",";
            strids+=$(this).parents("tr").find("td:eq(1)").text()+"-";
        })
        yhnames.substring(0,yhnames.length-1);
        strids= strids.substring(0, strids.length-1)
        if(confirm("确认删除>" + yhnames + "<?")){
            $.ajax({
                url: "${APP_PATH}/yhCRUD/" + strids,
                type: "DELETE",
                success: function (result) {
                    to_page(currentPage);
                }
